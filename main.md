...
**表１ MNBP諸元 （バルーンの素材に和紙＋こんにゃくを用いた場合）**
...
直径4メートル, 厚さ10ミリメートルのパイプをスライスしてフレームとしたいが（バルーンの半径は5メートル）、その外側に水耕栽培モジュールを取り付ける想定で強度に問題がないか確認する。
まずは、ナイロン製のフレームの強度を知りたいのだが、そのために周方向のフープ応力を求める。
[内圧を受ける薄肉円筒に生じる応力（フープ応力） - 製品設計知識](https://seihin-sekkei.com/calculation-tool/internal_pressure_thin_cylinder/)
...
<!-- パイプ内側の面積 = (直径)2[m] * π * (幅)1[m] = 2π [m^2]
P = 1117.9581 / (2π) = 177.928557784623... ≒ 178 [Pa]
σ = (PD)/(2t) = (178 *  -->

ここで、パイプの幅を仮に1メートルとすると、
パイプ内側の面積 = (直径)4[m] * π * (幅)1[m] = 4π [m^2]
P = 1117.9581 / (4π) = 88.9642788923117... ≒ 89 [Pa] = 89 * 10^-6 [MPa]

内圧が求められたので、周方向のフープ応力を計算する。
σ = (PD)/(2t) = (89 * 10^-6 * 4000)/(2 * 10)
  = 0.0178 [MPa]

ナイロンの引張強度は、41-166 \[\] の範囲なので 100 MPa とすると、
[PA6（ナイロン6）物性表｜KDAのプラスチック加工技術](https://www.kda1969.com/materials/pla_mate_pa6b.htm)

100 / 0.0178 = 5617.97752808989... ≒ 5600 倍の強度の余裕がある。
ナイロンの比重は、1.12-1.14 なので 1.13 として、必要な強度を満たすフレームの質量を計算すると、
<!-- ((2.005)^2 - 2^2) * π * 1 * 1.13 ÷ 5600 = 4.0174892859296 ≒ 4 [kg] -->
((2.005)^2 - 2^2) * π * 1 * 1.13 ÷ 5600 = 0.0000126944185649 [kg] ≒ 127 [mg]
となる。

ナイロンでは剛性が足りない可能性があるので、鋼鉄（S45C）をフレーム素材とした場合の質量も計算する。
S45C の引張強度を 690, 比重を 7.85 として、
[S45C - 川上ハガネ株式会社](https://www.kawakamihagane.com/materials/s45c/)
強度の余裕は 5600 / 100 * 690 ≒ 38640 倍 なので、
((2.005)^2 - 2^2) * π * 1 * 7.85 ÷ 38640 = 0.0000127807087001 [kg] ≒ 12.8 [mg]
```
